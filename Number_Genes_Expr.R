...
gExprd <- apply(tpmDF, 2, function(transcripts) length(subset(transcripts, transcripts > 1)))
mean(gExprd)


## Plot number of transcripts expressed per capture site after filtering for number
## of reads mapped
...
ggplot(df, aes(x = Total_Reads, y = gExprd, color = Dead.or.Alive...)) +
  ...
  ggtitle("Number_Transcripts_Expr.R
          ...
          n remaing = 37") +
  theme_grey(base_size = 12)
ggsave("../analysis/graphs/Number_Transcripts_Expr_Vs_Read_Depth.pdf")
