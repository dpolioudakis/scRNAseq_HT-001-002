...
for (l in 1:nlanes) {
    ...
    system(paste0("/geschwindlabshares/RNAseq_singlecellfetal/HT-001-002_DP/code/0.1_Pair.bash ", qseqdir[l], " ", outdir))
    ...
    for (i in 1:length(sampleinfo$Index_Name)) {
       ...
       ###################################
		...
		cat(paste0("/geschwindlabshares/RNAseq_singlecellfetal/HT-001-002_DP/code/0.2_fastq.R1.qual.convert.pl ", outdir, "/", sampleinfo$Index_Name[i], ".txt > ", outdir, "/", sampleinfo$Index_Name[i], ".R1.fastq\n"), file = paste0(outdir, "/Make_fastq_Sample_", sampleinfo$Index_Name[i], ".sh"), append = TRUE)
		cat(paste0("/geschwindlabshares/RNAseq_singlecellfetal/HT-001-002_DP/code/0.3_fastq.R2.qual.convert.pl ", outdir, "/", sampleinfo$Index_Name[i], ".txt > ", outdir, "/", sampleinfo$Index_Name[i], ".R2.fastq\n"), file = paste0(outdir, "/Make_fastq_Sample_", sampleinfo$Index_Name[i], ".sh"), append = TRUE)
	...
	system(paste0("qsub -S /bin/bash -N SC_", sampleinfo$Index_Name[i], " -o ", outdir, "/Make_fastq_Sample_", sampleinfo$Index_Name[i], ".log -e ", outdir, "/Make_fastq_Sample_", sampleinfo$Index_Name[i], ".log -pe shared 8 -j y -cwd -m n -q geschwind.q ", outdir, "/Make_fastq_Sample_", sampleinfo$Index_Name[i], ".sh"), intern = TRUE)
