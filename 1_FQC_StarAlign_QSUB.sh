...
# To submit this script: qsub -cwd -o logs/1_FQC_StarAlign_QSUB_$(date +%Y%m%d).log -e logs/1_FQC_StarAlign_QSUB_$(date +%Y%m%d).error -S /bin/bash -V -N SA_QSUB_1 -q geschwind.q -l h_data=4G,h_rt=3:00:00 1_FQC_StarAlign_QSUB.sh

# Reminder: make /logs directory in code directory
################################################################################
echo ""
echo "Starting 1_FQC_StarAlign_QSUB.sh..."$(date)
echo ""
################################################################################
...
for inDir in ${inParentDir}/Sxa*; do
  ...
  for inFastq1 in ${inDir}/${wildC}; do
    ...
    if [ ! -d ${outDir} ]; then
      ...
      outSampLog=logs/1_FQC_StarAlign_$(date +%Y%m%d)_${sampleID}.log
      outSampErr=logs/1_FQC_StarAlign_$(date +%Y%m%d)_${sampleID}.error

      # Note name of job (-N) will only display 1st 10 characters
      qsub -cwd -o ${outSampLog} -e ${outSampErr} -V -S /bin/bash -N A1_${sampleID} -q geschwind.q -l h_data=64G,h_rt=12:00:00 -pe shared 8 1_FQC_StarAlign.sh $outDir $inFastq1 $inFastq2 $inDir $sampleID
...
echo ""
echo "End of 1_FQC_StarAlign_QSUB.sh... "$(date)
